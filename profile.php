...
if(isset($id) and getnickname($id) != '') {
	...
	elseif($action == "guestbook") {
		...
		// get squad id and name
		$squadID = safe_query("SELECT squadID FROM ".PREFIX."squads_members WHERE userID='".$id."'");
		$squad_num = mysql_fetch_row($squadID); 
		$squad_num = $squad_num[0];
		
		$squadNAME = safe_query("SELECT name FROM ".PREFIX."squads WHERE squadID='".$squad_num."'");
		$squad_name = mysql_fetch_row($squadNAME); 
		$squad_name = $squad_name[0];
